...
GSS_dem <- GSS_dem %>% select(!c(MNTLHLTH, STRESS, MATESEX, PAIDSEX, SEXFREQ))
...
GSS_opin <- GSS_opin %>% select(!c(TAX, WRKEARN, WRKENJOY))
...
# removing variables with less than 20% non-NA
na_column_dem <- data.frame(missing = colSums(is.na(GSS_dem)))
...
# removing rows with NA in more than 25% of columns
na_row_dem <- rowSums(is.na(dem_clean))

threshold <- ncol(dem_clean) * 0.25

dem_clean <- dem_clean[na_row_dem <= threshold, ]


# removing variables with less than 50% non-NA
na_column_dem <- data.frame(missing = colSums(is.na(dem_clean)))
na_column_dem$missing <- as.numeric(na_column_dem$missing)

columns_to_elim <- na_column_dem %>% filter(na_column_dem$missing >= 0.5 * nrow(dem_clean))   
columns_to_elim <- row.names(columns_to_elim)
columns_to_elim <- as.character(columns_to_elim)
...
columns_to_elim <- na_column_opin %>% filter(na_column_opin$missing >= 0.5 * nrow(opin_clean))   
columns_to_elim <- row.names(columns_to_elim)
columns_to_elim <- as.character(columns_to_elim)
...
threshold <- ncol(opin_clean) * 0.25

opin_clean <- opin_clean[na_row_opin <= threshold, ]


# removing variables with less than 70% non-NA
na_column_opin <- data.frame(missing = colSums(is.na(opin_clean)))
na_column_opin$missing <- as.numeric(na_column_opin$missing)

columns_to_elim <- na_column_opin %>% filter(na_column_opin$missing >= 0.30 * nrow(opin_clean))   
columns_to_elim <- row.names(columns_to_elim)
columns_to_elim <- as.character(columns_to_elim)
columns_to_keep <- setdiff(colnames(opin_clean), columns_to_elim)
opin_clean <- opin_clean %>% select(all_of(columns_to_keep))
...
# removing variables with less than 80% non-NA
...
columns_dem <- colnames(dem_clean)
columns_opin <- colnames(opin_clean)
columns_misc <- colnames(misc_clean)

setdiff(columns, columns_alt)

all_clean <- all_clean %>%
  mutate(across(all_of(c("BALLOT", "WRKSTAT", "WRKSLF", "OCC10", "INDUS10", "MARITAL", "DIVORCE", "WIDOWED", "SPWRKSTA", "SPWRKSLF", "SPOCC10", "SPIND10", "PAWRKSLF", "PAOCC10", "SEX", "RACE", "RES16", "REG16", "MOBILE16", "FAMILY16", "BORN", "PARBORN", "GRANBORN", "REGION", "RELIG", "RELIG16", "FUND16", "SPREL", "RACLIVE", "HEALTH", "WKSUB", "WKSUBS", "WKSUP", "UNEMP", "OWNGUN", "PHONE", "PAPRES10", "PASEI10", "PRESTG10", "SEI10", "SPPRES10", "SPSEI10", "ID_", "CLASS_", "UNION_", "PARTYID", "NATROAD", "NATSOC", "NATMASS", "NATPARK", "SPKATH", "COLATH", "LIBATH", "SPKRAC", "COLRAC", "LIBRAC", "SPKCOM", "COLCOM", "LIBCOM", "SPKMIL", "COLMIL", "LIBMIL", "SPKHOMO", "COLHOMO", "LIBHOMO", "CAPPUN", "GUNLAW", "COURTS", "HELPFUL", "FAIR", "TRUST", "GETAHEAD", "ABDEFECT", "ABNOMORE", "ABHLTH", "ABPOOR", "ABRAPE", "ABSINGLE", "ABANY", "XMARSEX", "HOMOSEX", "POLHITOK", "POLABUSE", "POLMURDR", "POLESCAP", "POLATTAK", "TAX", "FUND", "HAPPY", "HAPMAR", "SATJOB", "LIFE", "SATFIN", "FINALTER", "XMOVIE", "FEAR", "PISTOL", "SHOTGUN", "RIFLE", "MAWRKGRW", "EVPAIDSX")), factor))
