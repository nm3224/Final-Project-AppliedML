...
GSS_dem <- GSS_dem %>% select(!c(MNTLHLTH, STRESS, MATESEX, PAIDSEX, SEXFREQ, BALLOT, CONINC, CONRINC))
...
# removing variables with more than 40% NA
na_column_dem <- data.frame(missing = colSums(is.na(dem_clean)))
na_column_dem$missing <- as.numeric(na_column_dem$missing)

columns_to_elim <- na_column_dem %>% filter(na_column_dem$missing >= 0.40 * nrow(dem_clean))   
columns_to_elim <- row.names(columns_to_elim)
columns_to_elim <- as.character(columns_to_elim)
...
# removing variables with more than 53% NA
na_column_opin <- data.frame(missing = colSums(is.na(opin_clean)))
na_column_opin$missing <- as.numeric(na_column_opin$missing)

columns_to_elim <- na_column_opin %>% filter(na_column_opin$missing >= 0.53 * nrow(opin_clean))   
columns_to_elim <- row.names(columns_to_elim)
columns_to_elim <- as.character(columns_to_elim)
...
# removing variables with more than 30% NA
na_column_opin <- data.frame(missing = colSums(is.na(opin_clean)))
na_column_opin$missing <- as.numeric(na_column_opin$missing)

columns_to_elim <- na_column_opin %>% filter(na_column_opin$missing >= 0.3 * nrow(opin_clean))   
columns_to_elim <- row.names(columns_to_elim)
columns_to_elim <- as.character(columns_to_elim)
columns_to_keep <- setdiff(colnames(opin_clean), columns_to_elim)
opin_clean <- opin_clean %>% select(all_of(columns_to_keep))
...
all_clean <- left_join(all_clean, misc_clean, by = c("YEAR", "ID_", "CLASS_", "PARTYID", "POLVIEWS", "FUND16", "HEALTH", "PHONE")) 
...
# removing rows with NA in more than 20% of all columns
na_row_all <- rowSums(is.na(all_clean))

threshold <- ncol(all_clean) * 0.2
...
all_clean <- all_clean %>%
  mutate(across(all_of(c("WRKSTAT", "OCC10", "INDUS10", "MARITAL", "SPWRKSTA", "SPOCC10", "SPIND10", "PAOCC10", "RACE", "RES16", "REG16", "MOBILE16", "FAMILY16", "PARBORN", "GRANBORN", "REGION", "RELIG", "RELIG16", "FUND16", "SPREL", "HEALTH", "OWNGUN", "PHONE", "ID_", "CLASS_", "UNION_", "PARTYID", "NATROAD", "NATSOC", "NATMASS", "NATPARK", "COURTS", "HELPFUL", "FAIR", "TRUST", "GETAHEAD", "XMARSEX", "HOMOSEX", "TAX", "FUND", "HAPPY", "SATJOB", "LIFE", "SATFIN", "FINALTER", "PISTOL", "SHOTGUN", "RIFLE", "CONFINAN", "CONBUS", "CONCLERG", "CONEDUC", "CONFED", "CONLABOR", "CONPRESS", "CONMEDIC", "CONTV", "CONJUDGE", "CONSCI", "CONLEGIS", "CONARMY", "DEGREE", "SPDEG", "PADEG", "MADEG", "ETH1")), as.character))
...
dem1 <- dem_clean %>% select(1:22)
dem2 <- dem_clean %>% select(23:45)
dem3 <- dem_clean %>% select(46:69)
vis_dat(dem1)
vis_dat(dem2)
vis_dat(dem3)
...
frequency$ethnic <- factor(frequency$ethnic, ordered = T, levels = c("German", "English / Welsh", "Irish", "Italian", "African", "Scots", "Mexican", "NativeAm", "French", "Polish", "Russian", "Swede", "Norweg", "Dutch", "Other"))
frequency <- na.omit(frequency)
...
```{r}
remove <- ls()
remove <- remove[!remove %in% c("all_clean", "dem_clean", "opin_clean", "misc_clean")]
rm(list = remove)

levels(dem_clean$CLASS_) <- c("L1", "L2", "L3", "L4")

set.seed(1)
index <- createDataPartition(dem_clean$CLASS_, p = 0.7, list = FALSE)
...
xg_final_fit <- train(CLASS_ ~ .,
                ...
                tuneGrid = data.frame(nrounds = 150,    #ADJUST BASED ON ABOVE RESULTS
                          max_depth = 4,   
                          eta = 0.05,   
                          min_child_weight = 5,
                          subsample = 0.6,  
...
```{r}
rf_resamp <- trainControl(method = "boot", number = 15, classProbs = TRUE, summaryFunction = multiClassSummary)
