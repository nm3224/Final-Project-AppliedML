...
# dir: C:/Users/marlo/OneDrive/Desktop/Uni/S8/Applied Machine Learning (STAT3106)/final_project/all variables

setwd(
  "C:/Users/dshew/Spring_2024_Classes/applied_ml/Final-Project-AppliedML/all variables"
)
...
setwd(
  "C:/Users/dshew/Spring_2024_Classes/applied_ml/Final-Project-AppliedML/demographic"
)
...
setwd(
  "C:/Users/dshew/Spring_2024_Classes/applied_ml/Final-Project-AppliedML/opinion"
)
...
setwd(
 "C:/Users/dshew/Spring_2024_Classes/applied_ml/Final-Project-AppliedML/miscellaneous"
)
...
```{r}
# # change all <NA> to NA
# GSS_dem <- apply(GSS_dem, 2, function(x) { ifelse(x == "<NA>", NA, x) })
# GSS_opin <- apply(GSS_opin, 2, function(x) { ifelse(x == "<NA>", NA, x) })
# GSS_misc <- apply(GSS_misc, 2, function(x) { ifelse(x == "<NA>", NA, x) })
# typeof(GSS_dem)
...
dem_train <- na.omit(dem_train)
dem_test <- na.omit(dem_test)
levels(dem_train$CLASS_) <- c("L1", "L2", "L3", "L4")
levels(dem_test$CLASS_) <- c("L1", "L2", "L3", "L4")
```
...
xg_final_fit <- train(CLASS_ ~ .,
                ...
                method = "xgbTree",
                verbose = FALSE,
                trControl = xgb_resamp, 
                # tuneGrid = data.frame(nrounds = 200,    #ADJUST BASED ON ABOVE RESULTS
                #          max_depth = 8,   
                #          eta = 0.05,   
                #          min_child_weight = 5,
                #          subsample = 0.6,  
...
```{r}
svm_resample <- trainControl(method = "cv",
                         number = 5,
                         classProbs = TRUE,
                         summaryFunction = multiClassSummary)
...
SVM_RBF <- train(CLASS_ ~ ., 
                 data = dem_train,
                 
                 method = "svmRadial",
                 
                 trControl = svm_resample,
...
```{r}
svm_rbf_fit_final <- trainControl(method = "none",
                                 classProbs = TRUE)

svm_rbf_final <- train(CLASS_ ~ ., 
                 data = dem_train,
                 
                 method = "svmRadial",
                 
                 trControl = svm_rbf_fit_final,
                 
                 verbose = FALSE,
                 
                 # tuneGrid = data.frame(sigma = 0.1, C = 1),   # ADJUST WITH BEST PARAMS
                 
                 metric = "ROC")

# Training set results

svm_pred_train <- predict(svm_rbf_final, newdata = dem_train)

svm_train_results <- confusionMatrix(dem_train$CLASS_, svm_pred_train)

svm_train_results

# Test set results

svm_pred_test <- predict(svm_rbf_final, newdata = dem_test)

svm_test_results <- confusionMatrix(dem_test$CLASS_, svm_pred_test)
...
dnn_final <- train(CLASS_ ~ .,
                  data = dem_train,
                  method = "mlpML",
                  # tuneGrid = data.frame(layer1 = 25, layer2 = 30, layer3 = 0),  #ADJUST WITH BEST PARAMS
                  metric = "ROC",
...
print(confusionMatrix(dnn_pred_train, dem_test$CLASS_))
```
