...
GSS_dem <- GSS_dem %>% select(!c(MATESEX, PAIDSEX, SEXFREQ, BALLOT, CONINC, CONRINC, DEGREE, PADEG, MADEG, SPDEG, PAPRES10))
...
GSS_opin <- GSS_opin %>% select(!c(BALLOT, ABSINGLE))
...
# removing rows with NA in more than 5% of all columns
na_row_all <- rowSums(is.na(all_clean))

threshold <- ncol(all_clean) * 0.05
...
all_clean <- all_clean %>%
  mutate(across(all_of(c(factor_candidates, "WRKSTAT", "OCC10", "INDUS10", "SPWRKSTA", "SPOCC10", "SPIND10", "PAOCC10", "RES16", "REG16", "FAMILY16", "PARBORN", "GRANBORN", "REGION", "RELIG", "RELIG16", "SPREL", "ID_", "PARTYID", "ETH1")), as.character))

all_clean <- all_clean %>%
  mutate(across(all_of(c(factor_candidates, "WRKSTAT", "OCC10", "INDUS10", "SPWRKSTA", "SPOCC10", "SPIND10", "PAOCC10", "RES16", "REG16", "FAMILY16", "PARBORN", "GRANBORN", "REGION", "RELIG", "RELIG16", "SPREL", "ID_", "PARTYID", "ETH1")), factor))
...
frequency$ethnic <- factor(frequency$ethnic, ordered = T, levels = c("German", "English / Welsh", "Irish", "Italian", "African", "Mexican", "Scots", "NativeAm", "French", "Polish", "Swede", "Norweg", "Russian", "Dutch", "Other"))
frequency <- na.omit(frequency)
...
```{r}
remove <- ls()
remove <- remove[!remove %in% c("all_clean", "dem_clean", "opin_clean", "columns_dem", "columns_opin", "columns")]
rm(list = remove)


levels(all_clean$CLASS_) <- c("L1", "L2", "L3", "L4")

all_clean <- all_clean %>% relocate(all_of(columns_dem), .before = everything()) %>% relocate(all_of(columns_opin), .after = tail(columns_dem, n=1)) %>% relocate(all_of(c("ID_", "YEAR", "CLASS_")), .before = everything())
...
# only doing initial pre-processing here so we can split into variable subsets after but also draw on all variables when imputing NAs with k-nearest neighbor. 
...
dem_train <- train %>% select(any_of(columns_dem))
dem_test <- test %>% select(any_of(columns_dem))
...
# the remaining pre-processing steps are done on the variable subsets seperately
...
opin_train <- train %>% select(any_of(columns_opin))
opin_test <- test %>% select(any_of(columns_opin))
...
#### finalizing all_train

```{r}

blueprint <- recipe(CLASS_ ~ ., data = train) %>%
...
# the remaining pre-processing steps applied to the all variables subset.

blueprint_prep <- prep(blueprint, training = train)
train <- bake(blueprint_prep, new_data = train)
test <- bake(blueprint_prep, new_data = test)
```
...
xg_final_fit <- caret::train(CLASS_ ~ .,
                data = dem_train, 
                method = "xgbTree",
                verbose = FALSE,
...
xg_train_results <- caret::confusionMatrix(dem_train$CLASS_, xg_pred_train)
...
xg_test_results <- caret::confusionMatrix(dem_test$CLASS_, xg_pred_test)
...
xg_train_results <- caret::confusionMatrix(opin_train$CLASS_, xg_pred_train)
...
xg_test_results <- caret::confusionMatrix(opin_test$CLASS_, xg_pred_test)
...
xg_train_results <- caret::confusionMatrix(train$CLASS_, xg_pred_train)
...
xg_test_results <- caret::confusionMatrix(test$CLASS_, xg_pred_test)
...
```{r}
rf_resamp <- trainControl(method = "cv", number = 5, classProbs = TRUE, summaryFunction = multiClassSummary)
...
rf_train_results <- caret::confusionMatrix(dem_train$CLASS_, rf_pred_train)
...
rf_test_results <- caret::confusionMatrix(dem_test$CLASS_, rf_pred_test)
...
rf_train_results <- caret::confusionMatrix(opin_train$CLASS_, rf_pred_train)
...
rf_test_results <- caret::confusionMatrix(opin_test$CLASS_, rf_pred_test)
...
rf_train_results <- caret::confusionMatrix(train$CLASS_, rf_pred_train)
...
rf_test_results <- caret::confusionMatrix(test$CLASS_, rf_pred_test)
...
```{r}
dnn_final <- caret::train(CLASS_ ~ .,
                  data = dem_train,
                  method = "mlpML",
                  tuneGrid = data.frame(layer1 = 25, layer2 = 30, layer3 = 15),
                  metric = "ROC",
...
caret::confusionMatrix(dnn_pred_train, dem_train$CLASS_)
caret::confusionMatrix(dnn_pred_test, dem_test$CLASS_)


# Feature importance

perm_importance_all <- function(model, data, target) {
  # Store the original accuracy
  original_accuracy <- postResample(predict(model, data), data[[target]])[1]
  
  # Initialize a vector to store importance
  importances <- setNames(rep(NA, ncol(data) - 1), names(data)[names(data) != target])
  
  # Loop over all features except the target
  for (feature in names(importances)) {
    # Permute the feature
    data_perm <- data
    data_perm[[feature]] <- sample(data[[feature]])
    
    # Measure accuracy with the permuted feature
    permuted_accuracy <- postResample(predict(model, data_perm), data_perm[[target]])[1]
...
feature_importances <- perm_importance_all(dnn_final, dem_train, "CLASS_")

# Sort and select the top 20 features
top_features <- sort(feature_importances, decreasing = T)[1:20]

# Print the results
feature_data <- data.frame(Feature = names(top_features), Importance = top_features)

# Create the bar plot
plot <- ggplot(feature_data, aes(x = reorder(Feature, -Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "steelblue") +  # Draw bars
  coord_flip() +  # Flip coordinates to make the bar plot horizontal
  theme_minimal() +  # Minimal theme
  labs(title = "Feature Importance", x = "Feature", y = "Decrease in Accuracy") +  # Add labels
  geom_text(aes(label = sprintf("%.4f", Importance)), hjust = -0.1, color = "black", size = 3.5)  # Add text labels
...
dnn_pred_train <- predict(dnn_final, newdata = opin_train)
dnn_pred_test <- predict(dnn_final, newdata = opin_test)

caret::confusionMatrix(dnn_pred_train, opin_train$CLASS_)
caret::confusionMatrix(dnn_pred_test, opin_test$CLASS_)


# Feature importance

# Apply the function to your data
feature_importances <- perm_importance_all(dnn_final, opin_train, "CLASS_")
...
dnn_final <- caret::train(CLASS_ ~ .,
                  data = train,
                  method = "mlpML",
                  tuneGrid = data.frame(layer1 = 45, layer2 = 30, layer3 = 15),
                  metric = "ROC",
                  trControl = trainControl(method = "none", classProbs = TRUE), 
                  importance = "impurity")

dnn_pred_train <- predict(dnn_final, newdata = train)
dnn_pred_test <- predict(dnn_final, newdata = test)

caret::confusionMatrix(dnn_pred_train, train$CLASS_)
caret::confusionMatrix(dnn_pred_test, test$CLASS_)


# Feature importance

# Apply the function to your data
feature_importances <- perm_importance_all(dnn_final, train, "CLASS_")
